...
def main(mu_0, q_exit, lambda_0, gamma, tau, shape, theta, m, p_max, time):
    ...
    for i in np.arange(0.1, p_max, 0.1):
        ...
        for k in np.arange(0, m):
            ...
            df_result.loc[j, "Price average"] = np.average(df['price'])
            ...
            df_result.loc[j, "Driver arrival rate | Theoretic"] = Model_Queue.lambda_dynamic(p_low, i, p_bal, mu_0,
                                                                                             q_exit, tau, gamma,
                                                                                             lambda_0, shape)
            df_result.loc[j, "Expected earnings per ride | Agent"] = s.arrivedDrivers / s.clock * df_result.loc[
                j, "Price average"]
            df_result.loc[j, "Expected earnings per ride | Theoretic"] = Model_Queue.eta_(p_low, i, df_result.loc[
                j, "Driver arrival rate | Theoretic"], mu_0, theta, shape)
            df_result.loc[j, "Revenue | Agent"] = s.arrivedDrivers / s.clock * df_result.loc[j, "Price average"]
    ...
    df_result = df.groupby(df['Price i'], ).mean()
    ### PLOT ###
    Model_Queue.plotDynamicAgentLambda(df_result, p_max)
    Model_Queue.plotDynamicAgentRevenue(df_result, p_max)